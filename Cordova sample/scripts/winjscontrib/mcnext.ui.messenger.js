...
(function (UI) {
    ...
    UI.Messenger.SmartWorkerPath = '/scripts/winjscontrib/mcnext.ui.messenger.worker.js';
            ...
            if (name && messenger[name]) {
                try {
                    WinJS.Promise.as(messenger[name](data)).then(function (arg) {
                        messenger._send({ name: name, id: details.id, type: 'complete', sender: 'mcnext.ui.messenger', data: arg });
                    }, function (arg) {
                        messenger._send({ name: name, id: details.id, type: 'error', sender: 'mcnext.ui.messenger', data: arg });
                    }, function (arg) {
                        messenger._send({ name: name, id: details.id, type: 'progress', sender: 'mcnext.ui.messenger', data: arg });
                    });
                } catch (exception) {
                    messenger._send({ name: name, id: details.id, type: 'error', sender: 'mcnext.ui.messenger', data: { description: exception.description, message: exception.message, stack: exception.stack } });
                }
