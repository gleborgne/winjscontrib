    ...
    */
    this.GetDataByClusterIndex = function (index) {
        ...
        if (c != null
            && c.length < index
            && c[index] != null
            && c[index]._clusterInfo != null
            && c[index]._clusterInfo.dataIndices != null
            && c[index]._clusterInfo.dataIndices.length > 0) {

            for (var j = 0; j < c[index]._clusterInfo.dataIndices.length; j++) {
                result.push(_data[c[index]._clusterInfo.dataIndices[j]]);
            }
        }
    ...
    this.GetPinByClusterIndex = function (index) {
        ...
        if (i >= 0) {
            do {
                ...
                if (pin._clusterInfo != null && pin._clusterInfo.index == index) {
                    return pin;
                }
            }
            while (i--);
